...
if not HAS_ACCEL:
    pytest.skip("No XPU/CUDA accelerator available", allow_module_level=True)
...
class TestFloat8Common:
    ...
    def init_single_module(self) -> nn.Module:
        ...
        module = nn.Linear(16, 16, device=DEVICE_TYPE)
        self.broadcast_module(module)
        return module

    def init_multi_module(self) -> nn.Module:
        torch.manual_seed(42)
        module = nn.Sequential(*[MLP(16, device=DEVICE_TYPE) for _ in range(3)])
        self.broadcast_module(module)
        return module
    ...
    def init_transformer(
        ...
        module = Transformer(args).to(get_current_accelerator_device())
...
class TestFloat8MultiProcess(FSDPTest, TestFloat8Common):
    ...
    def world_size(self) -> int:
        return min(_device_count(), 2)

    @unittest.skipIf(_device_count() < 2, "requires at least 2 accelerator devices")
    ...
    def _test_transformer_parity(
        ...
        local_inp = torch.randint(
            0, ref_module.tok_embeddings.weight.size(0), (16, 16), device=DEVICE_TYPE
        )
        check_parity_no_mp(
            self,
            ref_module,
...
class Test2DParallelMultiThread(FSDPTestMultiThread, TestFloat8Common):
    ...
    def test_amax_allreduce_device_mesh(self):
        ...
        global_mesh = init_device_mesh(
            DEVICE_TYPE, (pp_size, dp_size), mesh_dim_names=("pp", "dp")
        )
        ...
        if self.rank in [0, 1]:
            ...
            hp_tensor = torch.randn(768, 32, device=DEVICE_TYPE)
...
class TestFloat8MultiThread(FSDPTestMultiThread, TestFloat8Common):
    ...
    @unittest.skipIf(not HAS_ACCEL, "no accelerator")
    ...
    def test_fp32_fp8_single_module_parity(self):
        ...
        for enable_fsdp_float8_all_gather, scaling_type_weight in choices:
            ...
            ref_module = ref_module.to(get_current_accelerator_device())
            module = convert_to_float8_training(
                module_fp32,
                config=float8_linear_config2,
            )
    ...
    def test_fp32_fp8_multi_module_parity(self):
        ...
        for enable_fsdp_float8_all_gather, scaling_type_weight in choices:
            ...
            module = self.init_multi_module().to(get_current_accelerator_device())
            ref_module = copy.deepcopy(module)
            ref_module = convert_to_float8_training(
                ref_module,
                config=float8_linear_config1,
    ...
    def test_bf16_mp_fp8_dynamic_multi_parity(self):
        ...
        ref_module_fp32 = copy.deepcopy(module).to(get_current_accelerator_device())
